...
import { wholeScript } from "./script.const.js";

export const Script = ({ who }) => {
  const [script, setScript] = useState(wholeScript[who - 1]);
  ...
  const showScript = () => {
    if (script[currentId]?.isChoice === "TRUE" && !shown) {
      handleSelected();
  ...
  const renderChat = (question, type, who, samechar, sametype) => {
    if (script[currentId].isChoice === "TRUE") {
